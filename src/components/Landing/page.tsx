    ...
    const fetchLandingDetails= async (code: string) => {
      try {
        ...
        if (error.response) {
          ...
          // Other errors
          console.error("An error occurred during landing fetch:", error.message);
        }
      }
    };
          ...
          <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            ...
            <div className="p-7">
              ...
              <div className="flex justify-end gap-4.5 dark:text-white">
                ...
                <button
                  className="flex w-1/2 h-3/4 md:h-full justify-center rounded bg-primary px-6 py-2 font-medium text-gray hover:bg-opacity-90"
                  type="submit"
                  onClick={(isEditLanding ? updateLanding : uploadLanding)}
                >
                  {loding ? (
                      (isEditLanding ? "Updating...": "Uploading...")
                  ) : (
                    (isEditLanding ? "Update" : "Upload")
                  )}
                </button>
              </div>
            </div>
