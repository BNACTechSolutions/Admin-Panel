...
const QRScanLogTable = () => {
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      try {
        const response = await api.get("/api/admin/qr-scans", {
          headers: {
  ...
  const applyFilters = () => {
    ...
    if (filterConfig.clientName) {
      filtered = filtered.filter(log =>
        log.clientId.name.toLowerCase().includes(filterConfig.clientName.toLowerCase())
      );
    }
  ...
  const downloadCSV = () => {
    ...
    link.setAttribute("download", "qr_scan_logs.csv");
    link.click();
  };
              ...
              filteredLogs.map((log, key) => (
                <tr key={key}>
                  ...
                  <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                    <p className="text-black dark:text-white">{log.clientId.name}</p>
                  </td>
                  <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
